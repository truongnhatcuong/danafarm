...
import { Loader2, QrCode, ShoppingBag, Truck } from "lucide-react";
...
export function CheckoutClient({
  ...
  }, [hydrated, activeUserId, items.length, orderSuccess, router]);
  ...
  const quote = useMemo(() => {
    if (!address)
      return { distanceKm: null, shippingFee: 0, isFreeShip: false };
  ...
  return (
    <div className="grid gap-6 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <section className="rounded-2xl border border-shop-border bg-white p-5 md:p-6">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-shop-text/65">
                  Bạn thanh toán bằng tiền mặt cho nhân viên giao hàng khi nhận
                  được đơn hàng.
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div>
                <p className="flex items-center gap-2 font-semibold text-shop-title">
                  <QrCode size={17} className="text-shop-main" /> Chuyển khoản
                  ngân hàng (Mã VietQR)
                </p>
                <p className="mt-1 text-sm text-shop-text/65">
                  Sau khi đặt hàng, mã QR VietQR cùng thông tin số tài khoản và
                  nội dung chuyển khoản sẽ hiển thị để bạn thanh toán nhanh
                  chóng.
                </p>
              </div>
            </label>
      ...
      <aside className="h-fit space-y-4 rounded-2xl border border-shop-border bg-white p-5 lg:sticky lg:top-5">
          ...
          {items.map((item) => (
            <div key={item.key} className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium text-shop-title">
                  {item.name}
                </p>
                {item.variantName && (
                  <p className="text-xs text-shop-text/55">
                    {item.variantName}
                  </p>
                )}
                <p className="text-sm font-semibold text-shop-main">
        ...
        <Link
          href="/cart"
          className="block text-center text-sm text-shop-text/60 hover:text-shop-main hover:underline"
        >
          Quay lại giỏ hàng
        </Link>
      </aside>
