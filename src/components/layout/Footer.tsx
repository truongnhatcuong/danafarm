...
import { SITE_INFO, POLICY_PAGES, NAV_ITEMS } from "@/lib/constants";
    ...
    return (
        <footer className="mt-16 bg-footer-bg-1 border-t border-shop-border">
            <Container className="py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
                <div>
                    <h3 className="font-bold text-footer-title mb-3">{SITE_INFO.companyName}</h3>
                    <p className="text-sm text-footer-text/80 mb-1">
                        MST: {SITE_INFO.taxCode}
                    </p>
                    <p className="text-sm text-footer-text/80 flex gap-2 mb-2">
                        <MapPin size={16} className="shrink-0 mt-0.5" />
                        <span>{SITE_INFO.addressBusiness}</span>
                    </p>
                    <p className="text-sm text-footer-text/80 flex gap-2">
                        <MapPin size={16} className="shrink-0 mt-0.5" />
                        <span>{SITE_INFO.addressHeadquarters}</span>
                    </p>
                </div>

                <div>
                    <h3 className="font-bold text-footer-title mb-3">Danh mục sản phẩm</h3>
                ...
                <div>
                    ...
                    <ul className="space-y-2 text-sm text-footer-text/80">
                        <li className="flex items-center gap-2">
                            <Phone size={16} />
                            <a href={`tel:${SITE_INFO.phone}`} className="hover:text-footer-hover">
                                {SITE_INFO.phone}
                            </a>
                        </li>
                        <li className="flex items-center gap-2">
                            <Mail size={16} />
                            <a href={`mailto:${SITE_INFO.email}`} className="hover:text-footer-hover">
                                {SITE_INFO.email}
                            </a>
                        </li>
                    </ul>
            ...
            <div className="bg-footer-bg-copyright py-4">
                <Container>
                    <p className="text-center text-xs text-footer-text/70">
                        © {new Date().getFullYear()} {SITE_INFO.companyName}. All rights reserved.
                    </p>
                </Container>
            </div>
        </footer>
