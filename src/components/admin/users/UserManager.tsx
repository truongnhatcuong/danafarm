...
type UserMeta = { page: number; pageSize: number; total: number; pageCount: number };
...
export function UserManager() {
    ...
    const [meta, setMeta] = useState<UserMeta>({ page: 1, pageSize: 10, total: 0, pageCount: 1 });
    ...
    const load = useCallback(
        async (page = 1) => {
            ...
            const res = await fetch(`/api/admin/users?${params.toString()}`).then((r) => r.json());
            setItems(res.data ?? []);
    ...
    return (
        <div className="space-y-6">
            <header className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 className="text-2xl font-bold text-admin-ink">Khách hàng & Người dùng</h1>
                    <p className="text-sm text-admin-muted">Danh sách tài khoản đã đăng ký trên hệ thống DanaFarm.</p>
                </div>
                <div className="flex items-center gap-2 rounded-xl border border-admin-border bg-admin-surface px-3 py-1.5 text-xs font-semibold text-admin-muted shadow-xs">
                    Tổng số: <span className="text-sm font-bold text-admin-ink">{meta.total}</span> tài khoản
                </div>
            </header>
            ...
            <section className="rounded-2xl border border-admin-border bg-admin-surface shadow-xs">
                                    ...
                                    <tr key={u.id} className="transition hover:bg-admin-bg/40">
                                        <td className="p-3.5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <strong className="block font-semibold text-admin-ink">{u.name}</strong>
                                                    <span className="text-xs text-admin-muted">ID #{u.id}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-3.5 text-xs text-admin-muted">
                                            <div className="flex items-center gap-1.5 text-admin-ink">
                                                <Mail size={13} className="text-admin-muted" /> {u.email}
                                            </div>
                                            {u.phone && (
                                                <div className="mt-1 flex items-center gap-1.5">
                                                    <Phone size={13} className="text-admin-muted" /> {u.phone}
                                                </div>
                                            )}
                                        </td>
                                        <td className="p-3.5">
                                        ...
                                        <td className="p-3.5 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <button
                                                    type="button"
                                                    title={u.role === "ADMIN" ? "Chuyển thành khách hàng" : "Phân quyền Quản trị"}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-admin-border p-4 text-xs text-admin-muted">
                    <span>
                        Hiển thị <strong>{items.length}</strong> / <strong>{meta.total}</strong> người dùng
                    </span>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-[2px]">
                    <div className="w-full max-w-sm rounded-2xl border border-admin-border bg-admin-surface p-5 shadow-2xl">
                        <h3 className="text-base font-bold text-admin-ink">Xác nhận xóa tài khoản?</h3>
                        <p className="mt-1 text-sm text-admin-muted">
                            Bạn có chắc muốn xóa tài khoản <strong>&ldquo;{deleteTarget.name}&rdquo;</strong> ({deleteTarget.email})?
                        </p>
