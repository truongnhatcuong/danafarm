...
import { ChevronLeft, ChevronRight, ImageOff, Pencil, Search, Trash2 } from "lucide-react";
import type { Category, CategoryMeta } from "./types";

const fieldClass =
    "w-full rounded-lg border border-admin-border bg-admin-surface px-3 py-2 text-sm text-admin-ink outline-none focus:border-admin-accent";
...
export function CategoryTable({
    ...
    return (
        <section className="rounded-2xl border border-admin-border bg-admin-surface">
            <div className="flex flex-wrap items-center gap-2 border-b border-admin-border p-4">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {items.length === 0 && (
                            <tr>
                                <td colSpan={5} className="p-10 text-center text-sm text-admin-muted">
                                    Không tìm thấy danh mục nào.
                                </td>
                            </tr>
                        )}
                        {items.map((item) => (
                            <tr key={item.id} className="border-b border-admin-border last:border-none">
                                <td className="p-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0">
                                            <p className="truncate font-semibold text-admin-ink">{item.name}</p>
                                            <p className="truncate text-xs text-admin-muted">{item.slug}</p>
                                        </div>
                                    </div>
                                </td>
