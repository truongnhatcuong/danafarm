    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <Link href="/admin/orders" className="mb-2 flex items-center gap-1.5 text-sm font-medium text-admin-muted hover:text-admin-ink">
                        <ArrowLeft size={15} /> Quay lại danh sách đơn hàng
                    </Link>
                    <h1 className="text-2xl font-bold text-admin-ink">Đơn hàng #{order.code}</h1>
                    <p className="text-sm text-admin-muted">Đặt lúc {new Date(order.createdAt).toLocaleString("vi-VN")}</p>
                </div>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                ...
                <div className="rounded-2xl border border-admin-border bg-admin-surface p-5 shadow-xs">
                    ...
                    <p className="text-sm text-admin-ink">
                        {order.paymentMethod === "COD" ? "Thanh toán khi giao hàng (COD)" : "Chuyển khoản ngân hàng"}
                    </p>
                    {order.distanceKm != null && (
                        <p className="text-sm text-admin-muted">Khoảng cách ước tính: ~{order.distanceKm}km</p>
                    )}
                    {order.note && <p className="mt-1 text-sm italic text-admin-muted">Ghi chú: {order.note}</p>}
                </div>
            </div>
                    ...
                    {order.items.map((item) => (
                        <div key={item.id} className="flex items-center gap-4 p-4">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                {item.variantName && <p className="text-sm text-admin-muted">{item.variantName}</p>}
                                <p className="text-sm text-admin-muted">
                                    {formatCurrency(item.price)} × {item.quantity}
                                </p>
                            </div>
                            <strong className="text-admin-ink">{formatCurrency(item.lineTotal)}</strong>
                        </div>
                    ))}
                </div>
